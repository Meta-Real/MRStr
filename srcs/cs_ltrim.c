 ...
 *
 * void mrstr_cs_ltrim(mrstr_p, mrstr_pc, mrstr_cstr)
...
void mrstr_cs_ltrim(mrstr_p res, mrstr_pc str, mrstr_cstr chrs)
{
    ...
    if (!(chrs && (clen = strlen(chrs))))
    {
        ...
        if (!MRSTR_DATA(res))
            mrstr_dbg_aloc_err("mrstr_cs_ltrim", MRSTR_LEN(str) + 1, );
    ...
    if (res == str)
    {
        ...
        if (!MRSTR_LEN(res))
        {
            ...
            if (!tdata)
                mrstr_dbg_aloc_err("mrstr_cs_ltrim", MRSTR_OFFSET(res) + 1, );
    ...
    if (!MRSTR_DATA(res))
        mrstr_dbg_aloc_err("mrstr_cs_ltrim", MRSTR_LEN(res) + 1, );
