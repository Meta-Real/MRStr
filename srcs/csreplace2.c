 ...
 *
 * void mrstr_csreplace2(mrstr_p, mrstr_pc, mrstr_cstr, mrstr_chr)
...
void mrstr_csreplace2(mrstr_p res, mrstr_pc str, mrstr_cstr olds, mrstr_cstr news)
{
    if (olds == news)
    {
        ...
        if (!MRSTR_DATA(res))
            mrstr_dbg_aloc_err("mrstr_csreplace2", MRSTR_LEN(str) + 1, );
    ...
    if (!olds)
        mrstr_dbg_lmch_err("mrstr_csreplace2", 0ULL, strlen(news), );

    if (!news)
        mrstr_dbg_lmch_err("mrstr_csreplace2", strlen(olds), 0ULL, );

    mrstr_size olen = strlen(olds);
    mrstr_size nlen = strlen(news);
    if (olen != nlen)
        mrstr_dbg_lmch_err("mrstr_csreplace2", olen, nlen, );
