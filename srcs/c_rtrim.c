 ...
 *
 * void mrstr_c_rtrim(mrstr_p, mrstr_pc, mrstr_chr)
...
void mrstr_c_rtrim(mrstr_p res, mrstr_pc str, mrstr_chr chr)
{
    ...
    if (res == str)
    {
        ...
        if (!i)
            mrstr_data_free("mrstr_c_rtrim");

        tdata = __mrstr_realloc(MRSTR_DATA(res) - MRSTR_OFFSET(res),
                                    i + MRSTR_OFFSET(res) + 1);
        if (!tdata)
            mrstr_dbg_aloc_err("mrstr_c_rtrim", i + MRSTR_OFFSET(res) + 1, );
    ...
    if (!MRSTR_DATA(res))
        mrstr_dbg_aloc_err("mrstr_c_rtrim", i + 1, );
