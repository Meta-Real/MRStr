 ...
 *
 * void mrstr_ctrim(mrstr_p, mrstr_pc, mrstr_chr)
...
void mrstr_ctrim(mrstr_p res, mrstr_pc str, mrstr_chr chr)
{
    ...
    if (res == str)
    {
        ...
        if (!tdata)
            mrstr_dbg_aloc_err("mrstr_ctrim", j + MRSTR_OFFSET(res) + 1, );
    ...
    if (!MRSTR_DATA(res))
        mrstr_dbg_aloc_err("mrstr_ctrim", j + 1, );
