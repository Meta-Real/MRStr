 ...
 *
 * void mrstr_n_ctrim(mrstr_p, mrstr_pc, mrstr_chr, mrstr_size)
...
void mrstr_n_ctrim(mrstr_p res, mrstr_pc str, mrstr_chr chr, mrstr_size len)
{
    ...
    if (!len)
    {
        ...
        if (!MRSTR_DATA(res))
            mrstr_dbg_aloc_err("mrstr_n_ctrim", MRSTR_LEN(str) + 1, );
    ...
    if (res == str)
    {
        ...
        if (!tdata)
            mrstr_dbg_aloc_err("mrstr_n_ctrim", j + MRSTR_OFFSET(res) + 1, );
    ...
    if (!MRSTR_DATA(res))
        mrstr_dbg_aloc_err("mrstr_n_ctrim", j + 1, );
