 ...
 *
 * void mrstr_nn_ctrim(mrstr_p, mrstr_pc, mrstr_chr, mrstr_size, mrstr_size)
...
void mrstr_nn_ctrim(mrstr_p res, mrstr_pc str, mrstr_chr chr, mrstr_size len1, mrstr_size len2)
{
    ...
    if (!(len1 || len2))
    {
        ...
        if (!MRSTR_DATA(res))
            mrstr_dbg_aloc_err("mrstr_nn_ctrim", MRSTR_LEN(str) + 1, );
    ...
    if (res == str)
    {
        ...
        if (i == MRSTR_LEN(res))
            mrstr_data_free("mrstr_nn_ctrim");
        ...
        if (!tdata)
            mrstr_dbg_aloc_err("mrstr_nn_ctrim", j + MRSTR_OFFSET(res) + 1, );
    ...
    if (!MRSTR_DATA(res))
        mrstr_dbg_aloc_err("mrstr_nn_ctrim", j + 1, );
