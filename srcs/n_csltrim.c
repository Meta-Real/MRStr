 ...
 *
 * void mrstr_n_csltrim(mrstr_p, mrstr_pc, mrstr_cstr, mrstr_size)
...
void mrstr_n_csltrim(mrstr_p res, mrstr_pc str, mrstr_cstr chrs, mrstr_size len)
{
    ...
    if (!(len && chrs && (clen = strlen(chrs))))
    {
        ...
        if (!MRSTR_DATA(res))
            mrstr_dbg_aloc_err("mrstr_n_csltrim", MRSTR_LEN(str) + 1, );
    ...
    if (res == str)
    {
        ...
        if (!MRSTR_LEN(res))
            mrstr_data_free("mrstr_n_csltrim");
        ...
        if (!tdata)
            mrstr_dbg_aloc_err("mrstr_n_csltrim", MRSTR_LEN(res) + MRSTR_OFFSET(res) + 1, );
    ...
    if (!MRSTR_DATA(res))
        mrstr_dbg_aloc_err("mrstr_n_csltrim", MRSTR_LEN(res) + 1, );
