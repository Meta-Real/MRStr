 ...
 *
 * mrstr_size *mrstr_find_all_chr(mrstr_size*, mrstr_pc, mrstr_chr)
 * Returns the indexes of the matched characters within the string and their count
 *
 * input reqs:
...
mrstr_size *mrstr_find_all_chr(mrstr_size *count, mrstr_pc str, mrstr_chr chr)
{
    ...
    if (!idxs)
        mrstr_dbg_aloc_err("mrstr_find_all_chr", MRSTR_DEF_IDX_LST_LEN * sizeof(mrstr_size), NULL);
    ...
    for (i = 0; i < MRSTR_LEN(str); i++)
        if (MRSTR_DATA(str)[i] == chr)
        {
            if (*count == alloc)
            {
                ...
                {
                    free(idxs);
                    mrstr_dbg_aloc_err("mrstr_find_all_chr", alloc * sizeof(mrstr_size), NULL);
                }
    ...
    if (*count != alloc)
    {
        ...
        {
            free(idxs);
            mrstr_dbg_aloc_err("mrstr_find_all_chr", *count * sizeof(mrstr_size), NULL);
        }
