 ...
 *
 * void mrstr_rtrim_chr(mrstr_p, mrstr_pc, mrstr_chr)
...
void mrstr_rtrim_chr(mrstr_p res, mrstr_pc str, mrstr_chr chr)
{
    ...
    if (res == str)
    {
        ...
        if (!i)
            mrstr_data_free("mrstr_rtrim_chr");

        mrstr_str t_data = __mrstr_das_realloc(MRSTR_DATA(res) - MRSTR_OFFSET(res),
                                               i + MRSTR_OFFSET(res) + 1);
        if (!t_data)
            mrstr_dbg_aloc_err("mrstr_rtrim_chr", i + MRSTR_OFFSET(res) + 1, );
    ...
    if (!MRSTR_DATA(res))
        mrstr_dbg_aloc_err("mrstr_rtrim_chr", i + 1, );
