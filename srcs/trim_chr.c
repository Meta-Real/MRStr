 ...
 *
 * void mrstr_trim_chr(mrstr_p, mrstr_pc, mrstr_chr)
...
void mrstr_trim_chr(mrstr_p res, mrstr_pc str, mrstr_chr chr)
{
    ...
    if (res == str)
    {
        ...
        if (!j)
            mrstr_data_free("mrstr_trim_chr");
        ...
        if (!t_data)
            mrstr_dbg_aloc_err("mrstr_trim_chr", j + MRSTR_OFFSET(res) + 1, );
    ...
    if (!MRSTR_DATA(res))
        mrstr_dbg_aloc_err("mrstr_trim_chr", j + 1, );
