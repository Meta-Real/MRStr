 ...
 *
 * void mrstr_n_csrtrim(mrstr_p, mrstr_pc, mrstr_cstr, mrstr_size)
...
void mrstr_n_csrtrim(mrstr_p res, mrstr_pc str, mrstr_cstr chrs, mrstr_size len)
{
    ...
    if (!(len && chrs && (clen = strlen(chrs))))
    {
        ...
        if (!MRSTR_DATA(res))
            mrstr_dbg_aloc_err("mrstr_n_csrtrim", MRSTR_LEN(str) + 1, );
    ...
    if (!MRSTR_DATA(res))
        mrstr_dbg_aloc_err("mrstr_n_csrtrim", i + 1, );
