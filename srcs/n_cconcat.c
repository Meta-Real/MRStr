 ...
 *
 * void mrstr_n_cconcat(mrstr_p, mrstr_pc, mrstr_size, mrstr_chr)
...
void mrstr_n_cconcat(mrstr_p res, mrstr_pc str, mrstr_size len, mrstr_chr chr)
{
    ...
    if (res == str)
    {
        ...
        if (!tdata)
            mrstr_dbg_aloc_err("mrstr_n_cconcat", len + MRSTR_OFFSET(res) + 2, );
    ...
    if (!len)
    {
        ...
        if (!MRSTR_DATA(res))
            mrstr_dbg_aloc_err("mrstr_n_cconcat", 2ULL, );
    ...
    if (!MRSTR_DATA(res))
        mrstr_dbg_aloc_err("mrstr_n_cconcat", len + 2, );
