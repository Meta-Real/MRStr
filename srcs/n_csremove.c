 ...
 *
 * void mrstr_n_csremove(mrstr_p, mrstr_pc, mrstr_size, mrstr_cstr)
...
void mrstr_n_csremove(mrstr_p res, mrstr_pc str, mrstr_size len, mrstr_cstr chrs)
{
    ...
    if (res == str)
    {
        ...
        if (!j && i == MRSTR_LEN(res))
            mrstr_data_free("mrstr_n_csremove");
        ...
        if (!tdata)
            mrstr_dbg_aloc_err("mrstr_n_csremove", MRSTR_LEN(res) + MRSTR_OFFSET(res) + 1, );
    ...
    if (!MRSTR_DATA(res))
        mrstr_dbg_aloc_err("mrstr_n_csremove", MRSTR_LEN(str) + 1, );
    ...
    if (!tdata)
        mrstr_dbg_aloc_err("mrstr_n_csremove", MRSTR_LEN(res) + 1, );
