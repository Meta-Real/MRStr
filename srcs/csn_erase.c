 ...
 *
 * void mrstr_csn_erase(mrstr_p, mrstr_pc, mrstr_size, mrstr_cstr)
...
void mrstr_csn_erase(mrstr_p res, mrstr_pc str, mrstr_size len, mrstr_cstr chrs)
{
    ...
    if (res == str)
    {
        ...
        if (!j && i == MRSTR_LEN(res))
        {
            ...
            if (!tdata)
                mrstr_dbg_aloc_err("mrstr_csn_erase", MRSTR_OFFSET(res) + 1, );
        ...
        if (!tdata)
            mrstr_dbg_aloc_err("mrstr_csn_erase", MRSTR_LEN(res) + MRSTR_OFFSET(res) + 1, );
    ...
    if (!MRSTR_DATA(res))
        mrstr_dbg_aloc_err("mrstr_csn_erase", MRSTR_LEN(str) + 1, );
    ...
    if (!tdata)
        mrstr_dbg_aloc_err("mrstr_csn_erase", MRSTR_LEN(res) + 1, );
