 ...
 *
 * void mrstr_cconcat(mrstr_p, mrstr_pc, mrstr_chr)
...
void mrstr_cconcat(mrstr_p res, mrstr_pc str, mrstr_chr chr)
{
    if (res == str)
    {
        ...
        if (!tdata)
            mrstr_dbg_aloc_err("mrstr_cconcat", MRSTR_LEN(res) + MRSTR_OFFSET(res) + 2, );
    ...
    if (!MRSTR_LEN(str))
    {
        ...
        if (!MRSTR_DATA(res))
            mrstr_dbg_aloc_err("mrstr_cconcat", 2ULL, );
    ...
    if (!MRSTR_DATA(res))
        mrstr_dbg_aloc_err("mrstr_cconcat", MRSTR_LEN(res) + 1, );
