 ...
 *
 * void mrstr_n_cstrim(mrstr_p, mrstr_pc, mrstr_cstr, mrstr_size)
...
void mrstr_n_cstrim(mrstr_p res, mrstr_pc str, mrstr_cstr chrs, mrstr_size len)
{
    ...
    if (!(len && chrs && (clen = strlen(chrs))))
    {
        ...
        if (!MRSTR_DATA(res))
            mrstr_dbg_aloc_err("mrstr_n_cstrim", MRSTR_LEN(str) + 1, );
    ...
    if (res == str)
    {
        ...
        if (i == MRSTR_LEN(res))
            mrstr_data_free("mrstr_n_cstrim");
        ...
        if (!tdata)
            mrstr_dbg_aloc_err("mrstr_n_cstrim", j + MRSTR_OFFSET(res) + 1, );
    ...
    if (!MRSTR_DATA(res))
        mrstr_dbg_aloc_err("mrstr_n_cstrim", j + 1, );
