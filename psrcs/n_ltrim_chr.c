 ...
 *
 * void mrstr_n_ltrim_chr(mrstr_p, mrstr_pc, mrstr_chr, mrstr_size)
...
void mrstr_n_ltrim_chr(mrstr_p res, mrstr_pc str, mrstr_chr chr, mrstr_size len)
{
    ...
    if (res == str)
    {
        ...
        if (!MRSTR_LEN(res))
            mrstr_data_free("mrstr_n_ltrim_chr");
        ...
        if (!t_data)
            mrstr_dbg_aloc_err("mrstr_n_ltrim_chr", MRSTR_LEN(res) + MRSTR_OFFSET(res) + 1, );
    ...
    if (!MRSTR_DATA(res))
        mrstr_dbg_aloc_err("mrstr_n_ltrim_chr", MRSTR_LEN(res) + 1, );
